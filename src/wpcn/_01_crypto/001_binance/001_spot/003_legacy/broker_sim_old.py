...
from wpcn.wyckoff.phases import detect_phase_signals, AccumulationSignal
...
def simulate(df: pd.DataFrame, theta: Theta, costs: BacktestCosts, cfg: BacktestConfig, mtf: List[str] | None = None, scalping_mode: bool = False, use_phase_accumulation: bool = True, max_exposure: float = 0.50) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    """
    백테스트 시뮬레이션

    Args:
        max_exposure: 총 노출 상한 (기본값 50%). 축적 + Phase C 합산 노출이 이를 초과하면 신규 진입 차단.
    """
    df = df.sort_index()
    mtf = mtf or []
    ...
    for i in range(n):
        ...
        if t in accum_by_time and pos_side == 0:  # 메인 포지션 없을 때만 축적
            for acc_sig in accum_by_time[t]:
                ...
                # === 총 노출 상한 체크 ===
                # equity_val 계산 (현재 시점)
                current_equity = cash + accum_total_qty * c
                if short_accum_total_qty > 0:
                    total_margin_now = sum(p.get('margin', 0) for p in short_accum_positions)
                    current_equity += total_margin_now
                current_exposure = (accum_total_qty * c) / (current_equity + 1e-12)
                if current_exposure + acc_sig.position_pct > max_exposure:
                    continue  # 노출 상한 초과 → 스킵

                fill_px = apply_costs(acc_sig.entry_price, costs, is_buy=True)  # 매수
                ...
                if acc_sig.side == 'long':
                    ...
                    # 숏 진입은 매도 (is_buy=False)
                    fill_px_short = apply_costs(acc_sig.entry_price, costs, is_buy=False)
                    qty_short = alloc_amount / fill_px_short
                    new_total_qty = short_accum_total_qty + qty_short
                    if new_total_qty > 0:
                        short_accum_avg_price = (short_accum_avg_price * short_accum_total_qty + fill_px_short * qty_short) / new_total_qty
                    ...
                    short_accum_positions.append({
                        "time": t, "price": fill_px_short, "qty": qty_short,
                        "event": acc_sig.event, "phase": acc_sig.phase,
                        "margin": margin_amount
                    })
                    trades.append({
                        "time": t, "type": "ACCUM_SHORT", "side": "short",
                        "price": fill_px_short, "event": acc_sig.event,
                        "phase": acc_sig.phase, "position_pct": acc_sig.position_pct
                    })
        ...
        if accum_total_qty > 0 and accum_avg_price > 0 and i > accum_last_entry_i:
            ...
            # 익절 검사 (고가가 TP에 도달)
            if h >= dynamic_tp:
                exit_px = apply_costs(dynamic_tp, costs, is_buy=False)  # 매도
            ...
            # 손절 검사 (저가가 SL에 도달)
                ...
                exit_px = apply_costs(dynamic_sl, costs, is_buy=False)  # 매도
                pnl_pct = (exit_px - accum_avg_price) / accum_avg_price * 100
                cash += accum_total_qty * exit_px
                trades.append({
        ...
        if short_accum_total_qty > 0 and short_accum_avg_price > 0 and i > short_accum_last_entry_i:
            ...
            # 숏 익절 검사 (저가가 TP에 도달)
            if l <= dynamic_tp:
                exit_px = apply_costs(dynamic_tp, costs, is_buy=True)  # 숏 청산은 매수
            ...
            # 숏 손절 검사 (고가가 SL에 도달)
                ...
                exit_px = apply_costs(dynamic_sl, costs, is_buy=True)  # 숏 청산은 매수
                pnl_pct = (short_accum_avg_price - exit_px) / short_accum_avg_price * 100
        ...
        if pos_side == 0 and pending_orders:
            ...
            for od in pending_orders:
                ...
                if l <= limit_px <= h:
                    # 롱 진입 = 매수, 숏 진입 = 매도
                    is_long = od["side"] == "long"
                    fill_px = apply_costs(limit_px, costs, is_buy=is_long)
                    pos_side = +1 if is_long else -1
        ...
        if pos_side != 0:
            ...
            if (pos_side == 1 and l <= stop_price) or (pos_side == -1 and h >= stop_price):
                # 롱 청산 = 매도 (is_buy=False), 숏 청산 = 매수 (is_buy=True)
                exit_px = apply_costs(stop_price, costs, is_buy=(pos_side == -1))
                ...
                if not tp1_done:
                    if (pos_side == 1 and h >= tp1) or (pos_side == -1 and l <= tp1):
                        # 롱 청산 = 매도, 숏 청산 = 매수
                        exit_px = apply_costs(tp1, costs, is_buy=(pos_side == -1))
                        frac = cfg.tp1_frac
                ...
                if cfg.use_tp2 and pos_side != 0:
                    if (pos_side == 1 and h >= tp2) or (pos_side == -1 and l <= tp2):
                        # 롱 청산 = 매도, 숏 청산 = 매수
                        exit_px = apply_costs(tp2, costs, is_buy=(pos_side == -1))
                        if pos_side == 1:  # 롱
                            cash = cash + pos_qty * exit_px
                        else:  # 숏
                ...
                if pos_side != 0 and cfg.max_hold_bars is not None:
                    if (i - entry_i) >= cfg.max_hold_bars:
                        # 롱 청산 = 매도, 숏 청산 = 매수
                        exit_px = apply_costs(c, costs, is_buy=(pos_side == -1))
                        if pos_side == 1:  # 롱
                            cash = cash + pos_qty * exit_px
                        else:  # 숏
                            cash = cash + pos_qty * entry_px + pos_qty * (entry_px - exit_px)
