...
def detect_scalping_signals_dynamic(df: pd.DataFrame, theta: Theta, use_dynamic: bool = True) -> List[EventSignal]:
    ...
    for i in range(50, n):  # Start from 50 for dynamic params
        ...
        if fib_long[i] and current_k < 0.50 and long_ok:
            entry = close
            stop = low - stop_mult * at
            tp1 = close + tp1_mult * at
        ...
        if i > 0 and current_k < 0.40 and current_k > stoch_k_arr[i-1] and stoch_k_arr[i-1] < 0.30 and long_ok:
            entry = close
        ...
        if fib_short[i] and current_k > 0.50 and short_ok:
            entry = close
            stop = high + stop_mult * at
            tp1 = close - tp1_mult * at
            tp2 = close - (tp1_mult + 0.4) * at
        ...
        if i > 0 and current_k > 0.60 and current_k < stoch_k_arr[i-1] and stoch_k_arr[i-1] > 0.70 and short_ok:
            entry = close
