"""
ETH-USDT Out-of-Sample Validation
==================================
...
사용법:
    python models/ETH-USDT/validate_oos.py
"""
...
def load_eth_data(years: List[str]) -> Optional[pd.DataFrame]:
    """ETH 5분봉 데이터 로드"""
    all_dfs = []
    for year in years:
...
def main():
    log("=" * 80)
    log("ETH-USDT OUT-OF-SAMPLE VALIDATION")
    log("=" * 80)
    log("\n[METHODOLOGY]")
    ...
    df_train_5m = load_eth_data(['2021', '2022', '2023'])
    ...
    log(f"COMPARISON:")
    log(f"-" * 40)
    log(f"  Original (Data Leakage): $+122,946")
    log(f"  Out-of-Sample (2024):    ${test_stats.get('total_pnl', 0):+,.2f}")

    gap = 122946.25 - test_stats.get('total_pnl', 0)
